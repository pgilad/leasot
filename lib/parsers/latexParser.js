...
module.exports = function({ customTags } = {}) {
    ...
    return function parse(contents, file) {
        ...
        // multiple lines
        const multilineRegex = /^[ |\t]*\\begin{comment}\s*@?(todo|fixme)(?!\w)\s*(?:\(([^)]*)\))?\s*:?\s*(.*?)(?:\s+\/([^\s]+)\s*)?\\end{comment}/gmis;
        while ((m = multilineRegex.exec(contents)) !== null) {
            ...
            let preceeding_lines = contents.slice(0, m.index);
            let line_no = preceeding_lines.split(/\r\n|\r|\n/).length;
