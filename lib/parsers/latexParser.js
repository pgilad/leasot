...
// characters right
const latexmlregex = /^\s*\\begin{comment}\s*@?(TODO|FIXME)(?!\w)\s*(?:\(([^)]*)\))?\s*:?\s*(.*?)(?:\s+\/([^\s]+)\s*)?\\end{comment}$/gmsi;
module.exports = function({ customTags } = {}) {
    ...
    return function parse(contents, file) {
        ...
        // multiple lines
        let multiLineMatch = latexmlregex.exec(contents);
        if (multiLineMatch) {
            ...
            preceeding_lines = contents.split(multiLineMatch)[0]
            line_no = preceeding_lines.split(/\r\n|\r|\n/).length
            // Now prepare the comment
            comment = commentsUtil.prepareComment(multiLineMatch, line_no, file);
            comments.push(comment);
        }
        return comments;
    };
