...
function parse(options) {
    ...
    var parsersNames = [].concat(originalParser.parserName);

    var includedFiles = originalParser.includedFiles || [];
    if (withInlineFiles) {
        includedFiles.forEach(function (ext) {
            parsersNames.push(parsersDb[ext].parserName);
        });
        parsersNames = uniq(parsersNames);
    }

    var parsed = parsersNames
        .map(function (parsersName) {
            var parserFactory = require('./parsers/' + parser);
            return parserFactory(parseOptions)
        })
        .concat(customParsers)
        .map(function (parser) {
