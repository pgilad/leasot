...
function parse(options) {
     ...
     * customParsers eg: {
     *      '.js': {
     *          parserName: 'javascriptParser'
     *          parserFactory: function(parseOptions) {
    ...
    var originalParser = customParsers[ext] || parsersDb[ext];
    var parsers = [originalParser];

    var includedFiles = originalParser.includedFiles || [];
    if (withInlineFiles) {
        includedFiles.forEach(function (ext) {
            parsers.push(parsersDb[ext]);
        });
        parsers = uniqBy(parsers, function (a, b) {
            return a.parserName === b.parserName;
        });
    }

    var parsed = parsers
        .map(function (parsers) {
            var parserFactory = parsers.parserFactory || require('./parsers/' + parserName);
